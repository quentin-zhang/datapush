...
@RestController
@Api(tags = "导出数据按年导出")
@RequestMapping("/esExportYear")
public class ExportEsYear {

    private String writeFilePath;
    //private String url = "E:/database/宁夏数据";
    private String url = "/home/develop/database/xxsdsdsd线下山东";
    ...
    public String export(@RequestBody ExportEsRequest exportEsRequest) throws Exception {
        try {
            ...
            List<Map<String,String>> listMonth = new ArrayList<>();
            for(int s1 = 2018 ; s1<2021;s1++){
                if(s1==2020){
                for (int s=1 ; s<11; s++){
                    Map<String,String> map = new HashMap<>();
                    map.put("year",s1+"");
                    map.put("month",s+"");
                    map.put("startTime", DateUtil3.getFirstDayOfMonth(s1,s)+" 00:00:00");
                    map.put("endTime",new SimpleDateFormat("yyyy-MM-dd").format(new SimpleDateFormat("yyyy-MM-dd").parse(DateUtil3.getLastDayOfMonth(s1,s))) + " 23:59:59");
                    listMonth.add(map);
                ...
                for (int s=1 ; s<13 ; s++){
                   ...
                   /* Calendar c = Calendar.getInstance();
                    c.set(Calendar.YEAR, s1); // 2010年
                    c.set(Calendar.MONTH, s-1);

                    int days =  c.getActualMaximum(Calendar.DAY_OF_MONTH);
                    for (int s3 =1;s3<days+1;s3++) {
                        String st = String.valueOf(s);
                        String st1 = String.valueOf(s3);
                        Map<String,String> map = new HashMap<>();
                        map.put("year",s1+"");
                        map.put("month",s+"");
                        if(s<10){
                        ...
                        map.put("startTime",s1+"-"+st+"-"+st1 +" 00:00:00");
                        map.put("endTime",s1+"-"+st+"-"+st1 + " 23:59:59");
                        listMonth.add(map);
                    }
            ...
            for (int dateMonth = 0 ; dateMonth<listMonth.size(); dateMonth++ ) {
                int form = 100000;

                EsFUtil esl = new EsFUtil();
                Map<String, String> mapl = new HashMap<>();
                mapl.put("keytype", "17");
                mapl.put("startTime", listMonth.get(dateMonth).get("startTime"));
                mapl.put("endTime", listMonth.get(dateMonth).get("endTime"));
                ...
                if (StringUtils.isNotBlank(exportEsRequest.getAreaCode()))
                    mapl.put("areaCodes", exportEsRequest.getAreaCode());
                ...
                for (int aa = 0; aa < listl.size(); aa++) {
                    ...
                    map.put("startTime", listMonth.get(dateMonth).get("startTime"));
                    map.put("endTime", listMonth.get(dateMonth).get("endTime"));
                    if (StringUtils.isNotBlank(exportEsRequest.getSourceTypes()))
                        map.put("sourceTypes", exportEsRequest.getSourceTypes());
                    if (StringUtils.isNotBlank(exportEsRequest.getAreaCode()))
                        map.put("areaCodes", exportEsRequest.getAreaCode());
                ...
                if(System.getProperties().getProperty("os.name").indexOf("Windows")>0){
                    ...
                    String path = url + "/" + listMonth.get(dateMonth).get("endTime").substring(0, 4) + "/" + listMonth.get(dateMonth).get("startTime").substring(0, 10) + "到" + listMonth.get(dateMonth).get("endTime").substring(0, 10) + "/";
                    log.info("path：" + url);
